...
function evaluateExperiment(context, experiment) {
  ...
  const { identityNamespace } = experiment;
  const _a = experiment.randomizationUnit;
  const randomizationUnit = _a === void 0 ? RandomizationUnit.VISITOR : _a;
...
function traverseDecisionTree(decisionNodesMap, context, currentNodeId) {
  const _a = decisionNodesMap[currentNodeId];
  const { experiment } = _a;
  const { type } = _a;
